...
public class No802FindEventualSafeStates {
    ...
    public boolean isSafeNode(int node, int[] color) {
        if (color[node] > 0) {
            return color[node] == 2;
        }
        ...
        for (int nei : graph[node]) {
            ...
            if (color[nei] == 1 || !isSafeNode(nei, color)) {
                return false;
            }
        }
    ...
    private boolean isSafeNode(int node) {
